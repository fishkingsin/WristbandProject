...
public class StatisticLandscapeActivity extends LandscapeActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		int numBars = 100;
		RoundBarGraphView mGraphView = new RoundBarGraphView(this, "Statistic");
		mGraphView.setHorizontalLabels(new String[] {
				getString(R.string.start), getString(R.string.end) });
		mGraphView.setVerticalLabels(new String[] { "90",
				"30" , "0" });

		GraphViewData data[] = new GraphViewData[numBars];
		for (int i = 0; i < numBars; i++) {
			data[i] = new GraphViewData(i, random.nextInt(90));
		}
