		...
		if (cal.size() > 0) {
			...
			try {
				Utilities.getLog(TAG,
						"First Date "+Utilities.getSimpleDateForamt().parse(cal.get(0))
						+"\nLastDate"+Utilities.getSimpleDateForamt().parse(cal.get(1)));
...
//		Utilities.targetDate().setTime(Utilities.lastDate().getTime());
	...
	protected void onReadDataFailed(boolean isPedometerStart,
		...
		if(! isSleepDataStart || !isSleepDataEnd)Log.e(TAG,"onReadSleepDataFailed");
